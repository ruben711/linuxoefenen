 ...
 *  Insane = 0 XP (enkel eer). Retries geven nooit XP. */
  ...
  persist(
    (set, get) => ({
      ...
      badges: () => {
        ...
        const out: string[] = [];
        const solvedExs = all.filter((e) => st.solved[e.id]);
        const hasTag = (t: string) => solvedExs.some((e) => (e.tags ?? []).includes(t));
        ...
        // Bash-flavor (tag-gebaseerd)
        if (hasTag("pipe")) out.push("🔧 Pipe-meester");
        if (hasTag("permissions")) out.push("🔐 chmod-goeroe");
        if (hasTag("script")) out.push("📜 Scripter");
        if (hasTag("regex") || hasTag("grep")) out.push("🧪 grep-detective");
        if (hasTag("find")) out.push("🗺️ find-navigator");
