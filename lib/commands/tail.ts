...
/** tail — toon de laatste N regels (standaard 10). -f wordt genegeerd (simulatie). */
export const tail: CommandHandler = ({ args, vfs, stdin }) => {
  ...
  if (p.pos.length === 0) {
    blocks.push(toLines(stdin).slice(-n).join("\n"));
  } else {
    for (const f of p.pos) {
      const r = vfs.readFile(f);
      if (r.err) { errs.push(`tail: ${r.err}`); continue; }
      const body = toLines(r.content!).slice(-n).join("\n");
      blocks.push(multi ? `==> ${f} <==\n${body}` : body);
    }
  }
